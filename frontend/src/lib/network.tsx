...
export const setAuthCookie = (authObj: any) => {
  Cookies.set(token, JSON.stringify(authObj));
};

export const setIsLoginCookie = (status: any) => {
  Cookies.set(isLogin, JSON.stringify(status));
};

export const parseCookie = (ctx: any) => {
...
export const getClientHeaders = (access: any) => {
  return {
    headers: {
...
export const getNewToken = async (e: any, refresh: any) => {
...
export const handleRouting = (path: any) => {
  Router.push(path);
};
...
export const getProducts = async (variables: any) => {
  const res = await client
    .query({
      query: productQuery,
      variables,
...
export const addToWish = (productId: any, dispatch: any) => {
